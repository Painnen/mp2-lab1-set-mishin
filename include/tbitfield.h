...
private:
  ...
  int *pMem; // память для представления битового поля
  int  MemLen; // к-во эл-тов Мем для представления бит.поля

  // методы реализации
  int   GetMemIndex(const int n) const; // индекс в pМем для бита n       (#О2)
  int GetMemMask (const int n) const; // битовая маска для бита n       (#О3)
  int GetBitIndex(const int n) const;
public:
  TBitField();
  ...
  void rBit(const int n);       // очистить бит                         (#П2)
